...
exports.GroupedAutoComplete = (0, react_1.forwardRef)((_a, ref) => {
    var { id, label, placeholder, color, variant, onChange, optionlabelkeyname, freeSolo, fieldSize, onBlur = () => { }, helperText, error, options, onTextFieldChange, limitTags, value, staticFilters, filterOptionsCallBack = (options, params) => {
    ...
    } } = _a, props = __rest(_a, ["id", "label", "placeholder", "color", "variant", "onChange", "optionlabelkeyname", "freeSolo", "fieldSize", "onBlur", "helperText", "error", "options", "onTextFieldChange", "limitTags", "value", "staticFilters", "filterOptionsCallBack"]);
    ...
    const handleOnChange = (event, newValue, reason) => {
        if (newValue.length && newValue[newValue.length - 1].type === 'filters') {
            const { title: filterName, value: filterValue } = newValue.find((value) => value.type === 'filters');
            ...
            const alreadySelected = Boolean(selectedOptions.find((option) => option.title === lastEl.title));
            if (alreadySelected) {
                newValue = newValue.filter((value) => value.title !== lastEl.title);
            }
        }
        if (reason === 'clear')
            setSelectedGroup([]);
    ...
    return ((0, jsx_runtime_1.jsx)(jsx_runtime_1.Fragment, { children: (0, jsx_runtime_1.jsx)(Autocomplete_1.default, Object.assign({}, props, { fullWidth: true, multiple: true, options: getAutoCompleteOptions(), ref: ref, id: id, onBlur: onBlur, onChange: handleOnChange, ListboxComponent: Virtualisation_1.ListboxComponent, PopperComponent: StyledPopper, disableCloseOnSelect: true, getOptionLabel: (option) => {
                ...
                return ((0, jsx_runtime_1.jsx)(TextField_1.TextField, Object.assign({ helperText: helperText, error: error, size: fieldSize }, restParams, { InputProps: Object.assign(Object.assign({}, restInputProps), { startAdornment: ((0, jsx_runtime_1.jsx)("div", Object.assign({ style: {
                                maxHeight: '140px',
                                overflowY: 'auto'
                            } }, { children: startAdornment }))) }), variant: variant, color: color, label: label, placeholder: placeholder })));
            }, renderOption: (props, option, state) => [Object.assign(Object.assign({}, props), { color, optionlabelkeyname, firstOptionIndex: staticFilters.length, selectedOptions, selectedGroup, optionsWithType }), option, state] })) }));
});
